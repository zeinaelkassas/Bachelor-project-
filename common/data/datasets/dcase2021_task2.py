...
class MachineNewDataSet(torch.utils.data.Dataset):

    def __init__(
            self,
            new_root=os.path.join(os.path.expanduser('~'), 'shared', 'DCASE2021', 'new_data')
    ):
        self.new_root = new_root
        self.newSamples = glob.glob(os.path.join(self.new_root, '**','**', '*.wav'))
        assert len(self.newSamples) > 0
    ...
    def __getitem__(self, item):
        ...
        newSample = self.newSamples[item].copy()
        info = load_info(newSample['path'])
        for k in info:
            newSample[k] = info[k]
    ...
    def __len__(self):
        return len(self.newSamples)

if __name__ == '__main__':
    md = MachineDataSet()
    md_new = MachineNewDataSet()
